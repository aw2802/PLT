...
let main_method = ref stub_func

let mainWasDeclared = ref false
...
let convertToSast classes =
	...
	in
	let checkIfMainMethod func_decl = 
		if func_decl.fname  = "main" then (
			if !mainWasDeclared = true then begin
				raise (Failure ("main method declared more than once"));
				stub_func
			end
			else if func_decl.freturn <> JVoid then begin
				raise (Failure ("main method must have type void"));
				stub_func
			end
			else (
				mainWasDeclared := true;
				let sfunc_decl = convertMethodToSast func_decl;
				in
				sfunc_decl		
			)	
		)
			...
			stub_func
	in
	let checkMethod func_decl =
			main_method := checkIfMainMethod func_decl;
			(convertMethodToSast func_decl)::!function_list;
			convertMethodToSast func_decl
	...
	let convertCbodyToSast cbody =
		 ...
		 smethods = List.map checkMethod cbody.methods;
            	}
	...
	in
	let sclasses = List.map convertClassToSast classes
	in
	let sprogram = 
		{classes = sclasses;

...
let translate ast = 
	...
	let classes = ast.classes in
	let functions = ast.functions in
	let main = ast.main in 
	...
	let generate_class_struct c = 
		let class_t = Hashtbl.find class_type_table c.cname in
		let scope_list = List.map (function vdecl -> str_of_scope vdecl.vscope) c.cbody.variables in 
		let type_list = List.map (function vdecl -> get_llvm_type vdecl.vtype) c.cbody.variables in 
		let name_list = List.map (function vdecl -> vdecl.vname) c.cbody.variables in
		...
		List.iteri (
			fun i f ->
			let n = c.cname ^ "." ^ f ^ "_" ^ List.nth scope_list i in
	...
	let func_define fdecl = 
		
		let fscope = fdecl.fscope in
		let fname = fdecl.fname in
		let is_actual_param = ref false in
		let params = List.map (fun f -> get_llvm_type f.formal_type) fdecl.fformals in
		let fty = L.function_type (get_llvm_type fdecl.freturn) (Array.of_list params)
		in
	...
	let rec stmt_gen llbuilder = function
	  Block sl	 -> List.hd (List.map (stmt_gen llbuilder) sl)
	(*@TODO	| SVarDecl vdecl -> *)
	| Return (e,d)	 -> 
...
(** comment out above lines until for_gen and while_gen are defined **)
	...
	and expr_gen llbuilder = function
		  Int_Lit (i)		-> L.const_int i32_t i
		| Boolean_Lit (b)	-> if b then L.const_int i1_t 1 else L.const_int i1_t 0
		| Float_Lit (f)	-> L.const_float f_t f
		| String_Lit (s)	-> L.build_global_stringptr s "tmp" llbuilder
		| Char_Lit (c)		-> L.const_int i8_t (Char.code c)
	...
	let build_main main =
			...
			let _ = stmt_gen llbuilder (Block (main.body)) in 
